...
class DataEncoder(object):
    ...
    def transform_train_and_hidden(cls, train, hidden):
        ...
        train, hidden = cls.encode_train_and_hidden(train, hidden)
    ...
    def encode_train_and_hidden(cls, train, hidden):
        ...
        实现不encode '0'，之后必须重写
        """
        train, hidden, _ = cls.encode_column(train, hidden, ACCESSION_NUMBER)
        train, hidden, _ = cls.encode_column(train, hidden, ITEM_TYPE)
        train, hidden, _ = cls.encode_column(train, hidden, OBSERVABLE)
        return train, hidden
...
if __name__ == "__main__":
    ...
    train_df_list, hidden_df_list = DataEncoder.transform_train_and_hidden(
